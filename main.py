...
def update_sheet(sheet: gspread.Worksheet, df_new: pd.DataFrame):
    ...
    if records:
        ...
        df_current_dropped = df_current[~df_current['日付'].isin(
            df_new['日付'].unique())]
    ...
    sheet.clear()
    sheet.update('A1', [df_paste.columns.values.tolist()] +
                 df_paste.values.tolist())


def update(args, date_since: datetime.date = None):
    ...
    mf = MoneyforwardDriver(args.cookie_path)
    mf.login()
